...
exports.updateTampilan = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    if (error) {
      ...
      return res.status(500).json({ error: "Gagal mengubah status tampilkan" });
    }

    res.json({ message: "Status tampilkan berhasil diperbarui", data });
  } catch (err) {
